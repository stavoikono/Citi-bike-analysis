...
packages <- c("dplyr","data.table","ggplot2","tidyr", "MLDataR","tidymodels",
              ...
              "lubridate","ggmap","randomForest","RColorBrewer","janitor","plotly",
              "leaflet", "leaflet.extra")
...
```{r}
data_map_start <- data %>% select(lat=start_station_latitude, 
...
data_map_start %>% leaflet() %>% addTiles() %>% addMarkers(popup = ~name) %>%
  addHeatmap(lng=~long,lat=~lat,intensity=~sqrt(N),max = ~max(sqrt(N)),
             radius = 60, blur = 100) %>%
  addCircleMarkers(lng = ~long, lat=~lat, radius = ~sqrt(N),
                  fillOpacity = 0.1, opacity = 0.1, color = "red"
                  )
  

data_map_end <- data %>% select(lat=end_station_latitude, 
...
data_map_end %>% leaflet() %>% addTiles() %>% addMarkers(popup = ~popup) %>%
  addHeatmap(lng=~long,lat=~lat,intensity=~sqrt(N),max = ~max(sqrt(N)),
             radius = 60, blur = 100) %>%
  addCircleMarkers(lng = ~long, lat=~lat, radius = ~sqrt(N),
                  fillOpacity = 0.1, opacity = 0.1, color = "red"
